...
## Verified Research Delegation (Interactive Sessions)
- **Primary Thread Context Protection:** Never execute raw multi-query web searches or read multi-KB reference notes directly in the primary conversation thread.
- **Delegation:** When asked to research a topic or compile verified research into `03 Verified Research/` (whether de novo or synthesizing an existing chat), ALWAYS delegate to `subagent_type="verified-research"`.
- **Path A (De Novo):** Pass research question and target output path to `subagent_type="verified-research"`.
- **Path B (Synthesis):** Pass the conversation summary of key claims, quotes, and URLs to `subagent_type="verified-research"` with `mode: discussion_synthesis`.
- **Minimal Response:** Once the subagent finishes, return a concise 1-2 line confirmation with the note link.
